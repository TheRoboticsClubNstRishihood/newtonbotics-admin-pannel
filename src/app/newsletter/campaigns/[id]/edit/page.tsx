...
export default function EditCampaignPage({ params }: { params: { id: string } }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          excerpt: campaignData.excerpt || '',
          targetAudience: campaignData.targetAudience as 'all' | 'active' | 'inactive' | 'custom',
          template: campaignData.template as 'default' | 'news' | 'announcement' | 'event' | 'custom',
          tags: campaignData.tags || [],
          scheduledAt: campaignData.scheduledAt ? new Date(campaignData.scheduledAt).toISOString().slice(0, 16) : '',
          customEmails: campaignData.customEmails || [],
          excludeEmails: campaignData.excludeEmails || []
        });
      } else {
  ...
  const handleSendTest = async () => {
    alert('Test email functionality will be available after saving the campaign');
  };
  ...
  if (loading) {
    return (
      <AdminLayout pageTitle="Edit Campaign">
  ...
  if (campaign.status !== 'draft') {
    return (
      <AdminLayout pageTitle="Cannot Edit Campaign">
        <div className="text-center py-12">
          <h2 className="text-2xl font-bold text-gray-900">Cannot Edit Campaign</h2>
  ...
  return (
    <AdminLayout pageTitle={`Edit Campaign: ${campaign.title}`}>
